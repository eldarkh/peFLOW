...
namespace biot
{
  ...
  template <int dim>
  ExactSolution<dim>::ExactSolution(const double      cur_time,
                                    ParameterHandler &param)
    : Function<dim>(dim + 1 + dim * dim + dim + (3 - dim) * (dim - 1) +
                    (dim - 2) * dim)
    , current_time(cur_time)
    , prm(param)
    , exact_solution_val_data(dim + 1 + dim * dim + dim +
                              (3 - dim) * (dim - 1) + (dim - 2) * dim)
    , exact_solution_grad_val_data(
        dim *
        (dim + 1 + dim * dim + dim + (3 - dim) * (dim - 1) + (dim - 2) * dim))
  {}
  ...
  template <int dim>
  KInverse<dim>::KInverse(ParameterHandler &                    param,
                          const Functions::ParsedFunction<dim> *k_inv_data)
  ...
  public:
    ...
    double
    mu_value(const Point<dim> &p) const
    {
      return mu->value(p);
    }
    double
    lambda_value(const Point<dim> &p) const
    {
      return lambda->value(p);
    }
  ...
  template <int dim>
  LameCoefficients<dim>::LameCoefficients(
    ParameterHandler &                    param,
    const Functions::ParsedFunction<dim> *mu_data,
    const Functions::ParsedFunction<dim> *lambda_data)
    : Function<dim>(dim)
