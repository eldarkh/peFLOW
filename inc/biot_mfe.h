...
namespace biot
{
  ...
  private:
     ...
     */
    void
    copy_local_mat_to_global(const CellAssemblyCopyData &copy_data);
     ...
     */
    void
    copy_local_rhs_to_global(const CellAssemblyCopyData &copy_data);
    ...
    void
    output_results(const unsigned int cycle, const unsigned int refine);
     ...
     */
    const int total_dim =
      dim + 1 + dim * dim + dim + static_cast<int>(dim * (dim - 1) / 2);
